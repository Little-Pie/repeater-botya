...
data KeyBoard = KeyBoard {chat_id' :: Int
                         ,text' :: String
                         ,reply_markup' :: ReplyMarkup}

instance ToJSON KeyBoard where
    toJSON (KeyBoard aChatid aText aReplymarkup) = object ["chat_id" .= aChatid
                                                          ,"text" .= aText
                                                          ,"reply_markup" .= aReplymarkup]

data ReplyMarkup = ReplyMarkup [Keys]
                 | RemoveKeyboard

instance ToJSON ReplyMarkup where
    toJSON (ReplyMarkup keys) = object ["keyboard" .= [keys],
